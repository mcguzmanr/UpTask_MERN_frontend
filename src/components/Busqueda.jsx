...
const Busqueda = () => {
    ...
    const proyectosFiltrados = busqueda === '' ?  [] : proyectos.filter(proyecto => proyecto.nombre.toLowerCase(
                        ...
                        <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                            {proyectosFiltrados.map(proyecto => (
                                <Combobox.Option
                                    ...
                                    value={proyecto}
                                    // eslint-disable-next-line no-unused-vars
                                    className={({active}) => classNames('cursor-default', 'select-none px-4 pt-2', active && 'bg-sky-600', 'text-white')}
                                > 
                                    {proyecto.nombre}
                                </Combobox.Option>
                            ))}
